...
#         print(hybrid)
        ...
        if self.hybrid == False:
            # Patch embedding
            self.patch_embedding = nn.Conv2d(1, dim, kernel_size=(20, 8), stride=(20, 8))
        else:
    ...
    def forward(self, x):
        ...
        print(data.shape)
         Args:
            x (tensor): `b,c,fh,fw`
        """
        batch, num_images, channels, height, width = image_data.shape
        if self.hybrid == False:
            # Patch embedding based ImageTransformer
            x = DDFA.generate_uv_tex(x, self.patch_embedding, self.sequence_embedding, self.linear_1, self.proj, self.seq_embed, self.hybrid, self.variant, self.device)
        else:
            ...
            for batch_number in range(batch):
                ...
                for images in range((num_images)):
                    x = DDFA.generate_uv_tex(image_data[batch_number][images].unsqueeze(0), self.xception_feature_extractor, self.sequence_embedding, self.linear_1, self.proj, self.hybrid, self.variant, self.device)
                    x = x.transpose(2,1)
                    x = self.linear_3(x)
                    x = x.transpose(2,1)
